...
class Predator(Creature):
    ...
    def getState(self, creatures: list[Creature]) -> Tuple[Tuple[Tuple[Creature | Fruit, float, float]], Tuple[Tuple[Creature, float, float]], Tuple[float, float, float, float], Tuple[float, float]]:
        ...
        First element:  3-tuple of 2-tuples containing distX, distY to targets (e.g. [0][0][0] gets distance in X to first target)
        ...
        targets: list[Creature] = list(c for c in creatures if isinstance(c, Creature) and not isinstance(c, Predator))
    ...
    def setTargetDistChange(self, newTargets: list[Creature], targetsToCount=3, priority=2):
        ...
        if len(self.targetDistances) > 0 and len(newTargets) > 0:
            ...
            while len(self.targetDistances) > i and len(newTargets) > i and targetsToCount > i:
                scalingFactor = (targetsToCount - i) ** priority
